  ...
  try {
    ...
    if (query.player) {
      ...
      if (histRaw) {
        try {
          history =
            typeof histRaw === "string" ? JSON.parse(histRaw) : safeParse(histRaw);
    ...
    // ==========================================================
    // 📌 2) VŠETCI HRÁČI + SUMÁR (pre tabuľku vo frontende)
    // ==========================================================
    if (query.task === "all") {
      const playersRaw = (await redis.hgetall(M_PLAYERS)) || {};
      const parsed = {};
      let totalProfit = 0;
    ...
    if (query.task === "daily") {
      ...
      for (const player of Object.keys(playersRaw)) {
